...
def post_drink(jwt):
    ...
    try:
        drink_title = request_data.get("title", None)
        ...
        abort(422, "Drink cannot be created")
...
def edit_drink(jwt, id):
    ...
    try:
        if "title" in request_data:
            drink.title = request_data.get("title", None)
        
        if "recipe" in request_data:
            drink.recipe = json.dumps(request_data.get("recipe", None))
...
def not_found(error):
    return (
        jsonify({
            ...
            "message": f'unauthorized : {error.error["description"]}', 
        }),
        error.status_code,
    )
...
def method_not_allowed(error):
    return (
        jsonify({
            "success": False, 
            "error": 403, 
            "message": f'forbiden : {error.description}'
        }),
        403,
    )